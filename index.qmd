...
> Joel Oskarsson. Source and figures:
...
#| fig-cap: "The same triangular lattice on a 200 x 60 domain, scaled per-axis (top) and uniformly (bottom). Both panels use `ax.set_aspect(1)`."
#| code-summary: "Show the code that draws this"

import sys
sys.path.insert(0, "figures")
from make_figures import raw_lattice, scale_per_axis, scale_uniform, edge_lengths, draw
import matplotlib.pyplot as plt

domain = (200.0, 60.0)
graph, positions, span = raw_lattice(12, 12)
old, new = scale_per_axis(positions, span, domain), scale_uniform(positions, span, domain)
l_old, l_new = edge_lengths(graph, old), edge_lengths(graph, new)

fig, axes = plt.subplots(2, 1, figsize=(9, 5))
draw(axes[0], graph, old,
     f"Per-axis scaling: edges {l_old.min():.2f} to {l_old.max():.2f} "
     f"(max/min = {l_old.max() / l_old.min():.3f})")
draw(axes[1], graph, new,
...
*The code behind every figure and number here is in the linked repository.*
